...
import ImageNameConverter from '@site/src/components/ImageNameConverter';
...
```bash
docker pull registry.gdut.edu.cn/registry.gitlab.com/gitlab-org/gitlab-runner:latest
```
...
| **GitLab Container Registry** | `registry.gitlab.com/foo/bar:latest` | `registry.gdut.edu.cn/registry.gitlab.com/foo/bar:latest` |
...
202.116.132.68 gitlab.registry.gdut.edu.cn
...
| **GitLab Container Registry** | `registry.gitlab.com/foo/bar:latest` | `gitlab.registry.gdut.edu.cn/foo/bar:latest` |
...
<Tabs groupId="container-platforms">
  <TabItem value="docker" label={<PlatformIcon name="docker" label="Docker" />} default>
...
<Tabs groupId="operating-systems">
  ...
  <TabItem value="buildx" label={<PlatformIcon name="docker" label="Docker Buildx" />}>

Docker Buildx 构建时**不会**读取 Docker Daemon 的 `registry-mirrors` 配置，需要通过 `buildkitd.toml` 为其指定镜像加速地址。

**创建配置文件**

将以下内容保存为 `buildkitd.toml`（Windows 保存到如 `C:\Users\<用户名>\buildkitd.toml`，macOS / Linux 保存到如 `~/buildkitd.toml`）：

```toml title="buildkitd.toml"
[registry."docker.io"]
  mirrors = ["https://docker.registry.gdut.edu.cn"]

[registry."ghcr.io"]
  mirrors = ["https://ghcr.registry.gdut.edu.cn"]

[registry."quay.io"]
  mirrors = ["https://quay.registry.gdut.edu.cn"]

[registry."registry.k8s.io"]
  mirrors = ["https://k8s.registry.gdut.edu.cn"]

[registry."mcr.microsoft.com"]
  mirrors = ["https://mcr.registry.gdut.edu.cn"]

[registry."gcr.io"]
  mirrors = ["https://gcr.registry.gdut.edu.cn"]

[registry."nvcr.io"]
  mirrors = ["https://nvcr.registry.gdut.edu.cn"]

[registry."docker.elastic.co"]
  mirrors = ["https://elastic.registry.gdut.edu.cn"]

[registry."registry.gitlab.com"]
  mirrors = ["https://gitlab.registry.gdut.edu.cn"]
```

:::note[域名置换模式前置步骤]

Buildx 走的是域名置换模式，使用前需先完成 [Hosts 与 CA 证书配置](#配置-hosts-文件)。

:::

**创建 Builder 实例**

删除旧的 builder（如存在），然后用 `--buildkitd-config` 指定配置创建新的 builder：

<Tabs groupId="operating-systems">
  <TabItem value="win" label={<PlatformIcon name="windows" label="Windows" />}>
    ```powershell
    # 删除旧 builder（如存在）
    docker buildx rm gdut-builder 2>$null

    # 创建并应用配置
    docker buildx create --use --bootstrap `
      --name gdut-builder `
      --driver docker-container `
      --buildkitd-config "C:\Users\<用户名>\buildkitd.toml"
    ```
  </TabItem>
  <TabItem value="linux" label={<PlatformIcon name="linux" label="macOS / Linux" />}>
    ```bash
    # 删除旧 builder（如存在）
    docker buildx rm gdut-builder 2>/dev/null || true
...
`buildkitd.toml` 在 builder 容器创建时注入，已存在的 builder 无法热加载新配置，修改配置后需要删除并重建 builder。也可以将配置文件放在 `~/.docker/buildx/buildkitd.default.toml`，之后创建的新 builder 会自动使用。
...
```bash
# 查看 builder 状态
docker buildx inspect gdut-builder

# 测试构建
docker buildx build --builder gdut-builder -t test .
```
